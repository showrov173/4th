...
// 🔥 STRICT SERVER CHECK LOAD (সার্ভার ভেরিফিকেশন ছাড়া খুলবে না)
document.addEventListener("DOMContentLoaded", () => {
    let skeleton = document.getElementById('skeleton-loader');
    if(skeleton) skeleton.style.display = 'flex'; // লোডার চালু থাকবে
...
function checkSubscription() {
    ...
    if (!navigator.onLine) {
        let skeleton = document.getElementById('skeleton-loader');
        if(skeleton) skeleton.style.display = 'none';
        alert("⚠️ সার্ভারের সাথে কানেক্ট করতে ইন্টারনেট প্রয়োজন! দয়া করে ইন্টারনেট চালু করে পেজটি রিলোড দিন।");
        return; // ইন্টারনেট না থাকলে এখানেই আটকে যাবে
    }

    let adminPass = localStorage.getItem('smartpos_admin_pass') || "ADMIN-PRO-MAX";

    // ২. মাস্টার অ্যাডমিন বাইপাস
    ...
    // ৪. প্রো লাইসেন্স সার্ভার থেকে ভেরিফাই (মূল সিকিউরিটি)
    else {
        if(db) {
            db.collection('licenses').doc(currentShopID).get().then(doc => {
                if(doc.exists) {
                    if(doc.data().isActive === false) {
                        ...
                        // সার্ভার থেকে ভেরিফাইড হলে তবেই ড্যাশবোর্ড দেখাবে
                        loadDataAndSync(currentShopID);
                        showDashboard();
                    }
                } else {
                ...
                let skeleton = document.getElementById('skeleton-loader');
                if(skeleton) skeleton.style.display = 'none';
                alert("❌ সার্ভার কানেকশনে সমস্যা হচ্ছে! দয়া করে ইন্টারনেট চেক করুন।");
            });
        }
    }
}
    ...
    if(cartItems.length === 0) {
        ...
        cartItems.forEach((item, i) => { 
            ...
            <tr style="border-bottom:1px solid #f1f5f9;">
                <td style="font-weight:600; color:#1e293b; font-size:15px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:150px;" title="${item.name}">${item.name}</td>
...
function applyShopBranding() {
    ...
    if (logoTextH2) {
        if (userPic && userPic.startsWith('data:image')) { 
            logoTextH2.innerHTML = `<img src="${userPic}" style="width: 30px; height: 30px; border-radius: 5px; object-fit: cover; vertical-align: middle; border: 2px solid #fff; margin-right: 5px; flex-shrink: 0;"> <span id="shop-name-display" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${shopNameText}</span>`; 
        } 
        else { 
            logoTextH2.innerHTML = `<i class="fa-solid fa-store" style="flex-shrink: 0;"></i> <span id="shop-name-display" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${shopNameText}</span>`; 
        }
    }
}
